...
class FlashCountTrial(dj.Computed):
	...
	def make(self, key):
		...
		if ("FreeS3" in bpod_data["trial_settings"].keys()):
			...
			trial_data["freerw"] = 0
		try:
			if ("Init" in bpod_data["states"]) and (not np.isnan(bpod_data["states"]["Init"][1])):
...
class DailySummary(dj.Manual):
    ...
    p_right : float                 # percentage of trials rats choose right 
    """
	...
	def _make_tuples(self, key):
		...
		if len(trial_datetime) > 0:
			...
			for d in unique_dates:
				...
				summary_data["p_right"] = sum(these_choices == "right")/len(these_trials)
